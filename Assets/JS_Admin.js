...
function editNav(navId) {
  $('.edit-nav' + navId)
    .addClass('editNavActive')
    .removeClass('editNavInactive')
    .siblings()
    .removeClass('editNavActive')
    .addClass('editNavInactive')
  $('.editDoctor-Child' + navId)
    .css({ display: 'flex' })
    .siblings()
    .css({ display: 'none' })
}
...
function FuncChart2(data = []) {
  ...
  if (!Array.isArray(data)) {
    console.warn(
      'FuncChart2(): Invalid data argument, expected array. Got:',
      data,
    )
    data = []
  }
...
function AddNewDoctor() {
  ...
  let hasError = false
  $(
    '#FirstNameWarning, #LastNameWarning, #GenderWarning, #SpecializationWarning, #warningRoom, #warningSchedule, #HMOWarning, #secretaryWarning, #categoryWarning',
  ).html('') // Clear previous warnings
...
function selectedItems(selectedIds, selectedId, SearchType) {
  var data = {
    functionSelectedItems:
      SearchType == 'Edit' ? EditSpecializationArr : selectedIds,
    ...
    success: function (response) {
      ...
      if (SearchType === 'Edit') {
        EditSpecializationArr.forEach(function (specId) {
          if (
            $('#hiddenInformationFieldIDSpecsEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#hiddenInformationFieldIDSpecsEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
      } else {
...
function selectedItems2(selectedIds, SearchType) {
  var data = {
    functionSelectedItems2:
      SearchType == 'Edit' ? EditSubSpecializationArr : selectedIds,
    selectedCode: SearchType,
  }
  $.ajax({
    url: '../Components/Function_Admin.php',
    ...
    success: function (response) {
      ...
      if (SearchType === 'Edit') {
        EditSubSpecializationArr.forEach(function (specId) {
          if (
            $('#hiddenInformationFieldIDSubSpecsEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#hiddenInformationFieldIDSubSpecsEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
    ...
    success: function (response) {
      ...
      if (SearchType === 'Edit') {
        EditNewRoomarr.forEach(function (specId) {
          if (
            $('#hiddenInformationFieldIDRoomEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#hiddenInformationFieldIDRoomEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
    ...
    success: function (response) {
      ...
      if (SearchType === 'Edit') {
        EditNewHMOarr.forEach(function (specId) {
          if (
            $('#hiddenInformationFieldIDHMOEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#hiddenInformationFieldIDHMOEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
...
function selectThisAddSubSpecs(
  selectThisAddSubSpecsName,
  selectThisAddSubSpecsID,
) {
...
function AddSchedule(Type) {
  const addDay =
    Type === 'FromEdit' ? $('#day-selectEdit').val() : $('#day-select').val()
  const addIn =
    Type === 'FromEdit' ? $('#pick-timeInEdit').val() : $('#pick-timeIn').val()
  const addOut =
    Type === 'FromEdit'
      ? $('#pick-timeOutEdit').val()
      : $('#pick-timeOut').val()
    ...
    success: function (response) {
      ...
      if (Type === 'FromEdit') {
        EditNewScheduleArr.forEach(function (specId) {
          if (
            $('#InformationFieldDoctorScheduleEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#InformationFieldDoctorScheduleEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
...
function AddSecretary(Type) {
  ...
  $.ajax({
    ...
    type: 'post',
    data: {
      AddSecretary: Type === 'FromEdit' ? EditedNewSecretaryArr : secretaryArr,
      Type: Type,
    },
    success: function (response) {
      if (Type === 'FromEdit') {
        ...
        EditedNewSecretaryArr.forEach(function (secretaryObject) {
          ...
          if (
            $('#InformationFieldAddSecretaryEdit').find(
              "[data-specid='" + specId + "']",
            ).length === 0
          ) {
            $('#InformationFieldAddSecretaryEdit').append(
              newItem.filter("[data-specid='" + specId + "']"),
            )
          }
        })
...
function isValidDayTime(value) {
  const regex =
    /^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s(0?[1-9]|1[0-2]):[0-5][0-9](AM|PM)\s-\s(0?[1-9]|1[0-2]):[0-5][0-9](AM|PM)$/i
...
function removeSelected(iconElement, specId, arrayType) {
  ...
  if (arrayType === 'DeleteSecretary') {
    if (EditedNewSecretaryArr.length > 0) {
      const index = EditedNewSecretaryArr.findIndex(
        (secretary) => secretary.number == specId,
      )
      if (index !== -1) {
  ...
  if (arrayType === 'SubSpecs') {
    ...
    const index = secretaryArr.findIndex(
      (secretary) => secretary.number == specId,
    )
    if (index !== -1) {
...
function PromptSubSpecialization(PromptSubSpecialization_ID) {
  ...
  if (EditSubSpecializationName === '') {
    $('#NewEditSubSpecializationWarning').html(
      'Please enter Sub-Specialization name.',
    )
    return
  }
...
function filterBySpecialization(specializationId) {
  var doctorName = $('#searchDoctor').val().trim()
  var data = {
...
function searchDoctor(doctorName) {
  var doctorName1 = doctorName.trim()
  var docSpecs = $('#filterSpecialization').val()
  var data = {
    searchDoctor: doctorName1,
    doctorSpecs: docSpecs,
  }
  $.ajax({
    url: '../Components/Function_Admin.php',
    type: 'post',
...
function searchHMO(HMOName) {
  var HMOName1 = HMOName.trim()
  var data = {
    searchHMO: HMOName1,
  }
  $.ajax({
    ...
    data: data,
    success: function (response) {
      $('.tbody-hmo').html(response)
    },
  })
}

function searchRoom(roomName) {
  var room = roomName.trim()
  var data = {
    searchRoom: room,
  }
  $.ajax({
    ...
    data: data,
    success: function (response) {
      $('.Room_tbl').html(response)
    },
  })
}

function searchSpecial(specName) {
  var spec = specName.trim()
  var data = {
    searchSpeci: spec,
  }
  $.ajax({
    ...
    data: data,
    success: function (response) {
      $('.spec-tbl').html(response)
    },
  })
}

function searchSubSpec(sspecName) {
  var sspec = sspecName.trim()
  var data = {
    searchSubSpeci: sspec,
  }
  $.ajax({
    url: '../Components/Function_Admin.php',
    type: 'post',
    data: data,
    success: function (response) {
      $('.subspeci-tbl').html(response)
    },
  })
}

function searchAcc(accName) {
  var acc = accName.trim()
  var data = {
    searchAcc: acc,
  }
  $.ajax({
    ...
    data: data,
    success: function (response) {
      $('.account-tbody').html(response)
    },
  })
}

function searchArch(archName) {
  var arch = archName.trim()
  var data = {
    searchArch: arch,
  }
  $.ajax({
